...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Max Pzap Supply
    uint256 public constant MAX_SUPPLY_CAP = 21000000 ether;
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        if (IERC20(pZap).totalSupply() + totalLockedUpRewards  >= MAX_SUPPLY_CAP) {
            return 0;
        }
    ...
    function pendingPZap(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 pZapReward =
                multiplier.mul(pZapPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    function updatePool(uint256 _pid) public {
        ...
        pZap.mint(devAddr, pZapReward.div(10));
        pZap.mint(address(this), pZapReward);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            pZap.mint(referrer, refBonusEarned);
